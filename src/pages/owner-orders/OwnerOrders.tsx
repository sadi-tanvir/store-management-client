...
import { GET_ORDERS, GET_ORDERS_BY_USER_ID } from '../../gql/queries/orderQueries';
...
const OwnerOrders = () => {
    ...
    // gql
    const orderResponse = useQuery(GET_ORDERS_BY_USER_ID, {
        variables: {
        ...
        <>
            <div className={`w-full min-h-screen ${classes.global_background}`}>
                <div className={`min-h-screen bg-slate-200 opacity-[0.80] pt-24`}>
                    ...
                    {
                        orderResponse?.data?.ownerOrders.length <= 0 ?
                            <h1 className="text-3xl font-bold text-center mt-20 uppercase opacity-70">order not available</h1>
                            :
                            <TableHeader headers={["email", "quantity", "Amount", "payment", "delivery", "actions"]}>
                                {
                                    orderResponse?.data?.ownerOrders?.map((order: ManageOrderType, index: number) => {
                                        return (
                                            <>
                                                <tr key={order._id}>
